...
def staticEstimationMidEndImproved(boardPos):
    ...
    whiteMoves=MiniMax.generateMovesMidEndGame(boardPos)
    if(numBlackPieces<=2): return 10000
    elif(numWhitePieces<=2): return -10000
    elif(blackMoves==0 or not blackMoves): return 10000
    elif(whiteMoves==0 or not whiteMoves): return -10000
    else:
        return (1000*(numWhitePieces-numBlackPieces)-len(blackMoves) + len(whiteMoves) + 5*(numberOfMills(boardPos,'W'))-10*(numberOfMills(boardPos,'B')) )